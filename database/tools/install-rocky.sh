...
for command_name in mariadb openssl mktemp install awk grep php; do
...

{
  ...
  printf '# DNI MariaDB application connection. Generated by database/tools/install-rocky.sh.\n'
...
if [ -n "$MIGRATION_RUNNER" ]; then
  ...
  MIGRATION_RESULT="$(php "$MIGRATION_RUNNER")"
