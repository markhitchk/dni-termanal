...
async function discordRequest(endpoint, token, { method = "GET", body } = {}) {
  const response = await fetch(`${DISCORD_API}${endpoint}`, {
    ...
    headers: {
      ...
      "User-Agent": "DNI-Terminal-Role-ID-Puller/1.3"
    },
...
async function resolveGuildId(token, configuredGuildId) {
  ...
  if (guilds.length === 1) {
    console.log(`Guild ID not set; using the bot's only server: ${guilds[0].name} (${guilds[0].id})`);
    return guilds[0].id;
  }
  const choices = guilds.map((guild) => `${guild.name} (${guild.id})`).join(", ");
  throw new Error(`Guild ID is required because the bot is in multiple servers: ${choices}`);
}
...
async function main() {
  if (typeof fetch !== "function") throw new Error("This script requires Node.js 20 or newer.");

  const env = await loadDiscordEnvironment();
  const token = env.token;
  const configuredGuildId = (readArg("--guild") || env.guildId || "").trim();
  const dmUserId = (readArg("--dm-user") || process.env.DISCORD_DM_USER_ID || "1459731143472713922").trim();
  ...
  if (!token) throw new Error("DISCORD_BOT_TOKEN was not found in the process environment or known DNI .env files.");
  console.log(`Discord token source: ${describeDiscordEnvSource(env.tokenSource)}`);
  if (env.guildSource) console.log(`Discord guild source: ${describeDiscordEnvSource(env.guildSource)}`);
  ...
  for (const targetName of targetRoles) {
    ...
    if (matches.length === 0) missing.push(targetName);
      ...
      duplicates[targetName] = matches.sort((a, b) => b.position - a.position).map((role) => ({ id: role.id, position: role.position }));
  ...
  for (const roleName of targetRoles) {
    if (roles[roleName]) console.log(`${roleName.padEnd(38)} ${roles[roleName]}`);
    else if (duplicates[roleName]) console.log(`${roleName.padEnd(38)} DUPLICATE NAME`);
    else console.log(`${roleName.padEnd(38)} MISSING`);
  }
  console.log("=".repeat(72));
