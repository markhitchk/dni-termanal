...
for(const file of ['public/index.html','public/src/html/index.html']){
  ...
  for(const required of [...expectedTabs,'WELCOME','RESEARCHER','DNI TERMINAL','dni-helmet.webp',expectedTitle,'MOCK / SIMULATION','Communication Nets','Connected Personnel','Operations Controls','Live Activity']){if(!html.includes(required)){console.error(`${file} missing ${required}`);process.exit(1);}}
...
for(const marker of ['MOCK / SIMULATION','DNI-MOCK-01','createMockNet','assignMockUser','startMockReadyCheck','sendMockAcars','simulateMockPulse']){if(!provider.includes(marker)){console.error(`Mock communications provider missing ${marker}`);process.exit(1);}}
if(/fetch\s*\(|XMLHttpRequest|EventSource\s*\(/.test(provider)){console.error('Mock provider must not make network requests');process.exit(1);}
console.log('DNI five-tab site and Star Comms mock communication module verification passed.');
