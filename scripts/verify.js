...
for(const file of ['public/index.html','public/src/html/index.html']){
  ...
  for(const required of [...expectedTabs,'WELCOME','RESEARCHER','DNI TERMINAL','dni-helmet.webp',expectedTitle]){if(!html.includes(required)){console.error(`${file} missing ${required}`);process.exit(1);}}
...
const css=fs.readFileSync('public/src/css/style.css','utf8');for(const marker of ['.welcome-title','.terminal-frame','.terminal-window','.hero-action.primary','background-size:48px 48px','.brand-logo','touch-action:pan-x','scroll-snap-type:x proximity','data-panel="sectors"']){if(!css.includes(marker)){console.error(`UI marker missing: ${marker}`);process.exit(1);}}
const script=fs.readFileSync('public/src/js/script.js','utf8');for(const marker of ["selectPanel('terminal'","selectPanel('dashboard'","selectPanel('services'","selectPanel('communication'","selectPanel('sectors'"]){if(!script.includes(marker)){console.error(`Missing DNI module handler: ${marker}`);process.exit(1);}}
if(!script.includes('API MODULE')){console.error('DNI Communication must remain reserved for the API module');process.exit(1);}
console.log('DNI five-tab branding, title, module states, images, and build verification passed.');
