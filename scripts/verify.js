...
for(const file of ['public/index.html','public/src/html/index.html']){
  ...
  for(const required of [...expectedTabs,'WELCOME','RESEARCHER','DNI TERMINAL','dni-helmet.webp',expectedTitle,'API CONTRACT / SIMULATION','STAR COMMS OWNER API','Communication Nets','Connected Personnel','Operations Controls','Live Activity','/api/v1/stream','responsive.css','mobile-large.css']){if(!html.includes(required)){console.error(`${file} missing ${required}`);process.exit(1);}}
...
for(const marker of ['max-width:700px','max-width:520px','max-width:380px','max-width:340px','min-height:48px','min-height:54px','.terminal-add','.hero-action','.metric-card','.roster-row','.net-select','.event-row']){if(!mobileLarge.includes(marker)){console.error(`Large mobile UI marker missing: ${marker}`);process.exit(1);}}
...
console.log('DNI responsive five-tab site, larger mobile controls, and documented Star Comms API-contract simulation verification passed.');
