...
export async function loadDiscordEnvironment() {
  ...
  const guildId = String(
    process.env.DISCORD_GUILD_ID
      || process.env.DNI_ROLE_EXPORT_GUILD_ID
      || process.env.DNI_DISCORD_GUILD_ID
      || ""
  ).trim();
  ...
  return {
    ...
    guildSource:
      sources.DISCORD_GUILD_ID
      || sources.DNI_ROLE_EXPORT_GUILD_ID
      || sources.DNI_DISCORD_GUILD_ID
      || (guildId ? "process environment" : null)
  };
}
