...
async function resolveGuildId(token, configured) {
  ...
  if (guilds.length !== 1) {
    throw new Error("Set DISCORD_GUILD_ID, DNI_ROLE_EXPORT_GUILD_ID, or DNI_DISCORD_GUILD_ID because the bot is installed in multiple servers.");
  }
  return guilds[0].id;
}
...
async function main() {
  ...
  const guildId = await resolveGuildId(token, env.guildId);
