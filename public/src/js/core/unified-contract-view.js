...
function applyUnifiedContractView() {
  ...
  if (heading?.textContent?.trim() === 'Main Bounty Board' && description) {
    description.textContent = 'One shared contract network. Organization affiliation is attached to each record; organizations do not create separate boards.';
  }
}

if (panel) {
  new MutationObserver(applyUnifiedContractView).observe(panel, {childList:true, subtree:true});
